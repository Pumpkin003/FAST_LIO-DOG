...
void Preprocess::hesai_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
    ...
    if(feature_enabled)
    {
      ...
      for (int i = 0; i < plsize; i++)
      {
        ...
        added_pt.z = pl_orig.points[i].z;
        added_pt.intensity = pl_orig.points[i].intensity;
        added_pt.curvature = (pl_orig.points[i].timestamp - time_head) * 1000.f; // units: ms
    ...
    else
    {
      for (int i = 0; i < plsize; i++)
      {
        ...
        added_pt.z = pl_orig.points[i].z;
        added_pt.intensity = pl_orig.points[i].intensity;
        added_pt.curvature = (pl_orig.points[i].timestamp - time_head) * 1000.f;  // curvature unit: ms // cout<<added_pt.curvature<<endl;
